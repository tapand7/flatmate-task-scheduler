...
export default function CreateTaskModal({ onClose, onCreated }: CreateTaskModalProps) {
  ...
  const handleSubmit = async (event: FormEvent) => {
    ...
    setError("");
    try {
      const response = await createTask({ ...form, flatId: user.flatId });
  ...
  return (
    <div className="fixed inset-0 z-40 grid place-items-center bg-slate-950/50 px-4" onClick={(event) => event.target === event.currentTarget && onClose()}>
      <section className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl dark:border dark:border-[#2D35D4]/30 dark:bg-[#101450]">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-slate-950 dark:text-white">Create task</h2>
            <p className="mt-1 text-sm text-slate-500 dark:text-[#AEB5FF]">The backend assigns it to the next active member.</p>
          </div>
          <button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-full border border-slate-300 text-slate-500 hover:bg-slate-100 dark:border-[#2D35D4]/50 dark:text-[#87CEEB] dark:hover:bg-[#1A1F8C]">
            x
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <label className="block">
            <span className="text-sm font-semibold text-slate-700 dark:text-[#DDE2FF]">Task title</span>
            <input
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700 dark:text-[#DDE2FF]">Description</span>
            <textarea
          ...
          <label className="block">
            <span className="text-sm font-semibold text-slate-700 dark:text-[#DDE2FF]">Due date</span>
            <input
              type="date"
          ...
          <div className="flex gap-3">
            <button type="button" onClick={onClose} className="flex-1 rounded-lg border border-slate-300 px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-100 dark:border-[#2D35D4]/50 dark:text-[#DDE2FF] dark:hover:bg-[#1A1F8C]">
              Cancel
            </button>
            <button type="submit" disabled={loading} className="flex-[2] rounded-lg bg-[#2D35D4] px-4 py-2.5 text-sm font-bold text-white hover:bg-[#1A1F8C] disabled:opacity-60">
